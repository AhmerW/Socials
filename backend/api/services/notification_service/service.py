...
from fastapi import FastAPI, WebSocket, status, BackgroundTasks, WebSocketDisconnect
import aiohttp
...
class NotificationManager():
    ...
    async def start(self):
        ...
        async for msg in self.consumer.client:
            data = self.clients.get(msg.value.get('uid'))
            if data:
                for con in data:
...
async def connectWs(websocket : WebSocket, ott: str, device: str):
    ...
    try:
        ...
        await ctx.ncm.removeUser(device)
